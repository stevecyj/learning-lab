...
2. 每項資料的單位是什麼？
3. 要使用哪個公式？
4. 結果要顯示到小數點後幾位？是否需要標示單位？

先把題目拆成這幾個步驟，比直接把公式寫成一長行更容易檢查。
...
在 `height_m` 不為零時，先除以 `height_m` 再乘回去，結果就是原本的 `weight_kg`，並不是 BMI。括號在這裡用來表達公式的結構。
...
這個版本讓使用者以公分輸入身高，再由程式轉成公尺。變數名稱也包含單位：
...
一般 Python 程式可以直接使用標準函式庫提供的 `math.pi`，不必自行寫入 `3.14`：
...
2. **名稱要表達資料意義**：`weight_kg`、`height_cm` 與 `radius` 比 `a`、`b` 或誤寫的 `width` 清楚。
3. **輸入需要轉型與驗證**：`input()` 回傳字串，而且使用者可能輸入文字、零或負數。
...
這兩個練習的重點，是把自然語言、單位與公式準確翻譯成程式。若結果和預期不同，應一併比對輸入、公式、實際輸出與手算過程。
