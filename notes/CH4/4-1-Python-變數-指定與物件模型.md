...
在 CPython 實作中，`id()` 的值就是物件的記憶體位址；但這只是 CPython 的做法，Python 並沒有規定其他實作也必須這樣處理。而且物件消失後，它的 `id` 數值之後可能被重複利用。
...
教材中「`a = "hello"` 與 `b = "hello"` 的 `id` 一定一樣」並不是 Python 規定一定會成立的行為。
...
4. **`id()` 只保證能辨認物件身分**：在物件存活期間，`id()` 會保持不變，並與同時存在的其他物件不同。CPython 直接使用記憶體位址作為 `id()`，但其他 Python 實作不一定這樣做。
...
10. Python 實作可以重用某些不可變物件，編譯器也可能共用常數；這些都只是實作方式或優化，Python 並沒有規定一定要這樣做。
