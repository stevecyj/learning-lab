...
這個區別在之後學習 `list`、`dict`、函式參數與物件共用時非常重要。
...
`type()` 與 `id()` 接收的是經由名稱取得的物件，不是在查詢一個獨立存在的「變數型別」。
...
因此實作程式時，不要把 `id()` 當成業務資料的編號，也不要依賴其數值大小或持久性。
...
日常資料比較通常使用 `==`。`is` 最常見的正確用法是和 `None` 比較：
...
因此 Python 常被形容為「動態型別且強型別」：型別在執行時確定，但不相容的型別並不會任意混在一起運算。
...
4. **`id()` 的語言保證是物件身分**：只有 CPython 實作將其直接對應到記憶體位址。
...
5. **清楚命名比 `a`、`b`更重要**：教學實驗可以用短名稱；真實程式應使用 `user_count`、`total_price` 等表達意義的名稱。
...
如果這個模型建立正確，之後學 list、dict、函式參數與物件導向程式時，就能預測「修改一個名稱看到的物件，為什麼另一個名稱也可能受影響」。
