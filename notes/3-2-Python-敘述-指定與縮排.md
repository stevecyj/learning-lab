...
「每一行都是一個敘述」可以當作初步理解，但並不完全準確：

- 一個敘述可以跨越多個實體行。
- 一行也可以用分號隔開多個簡單敘述，但可讀性較差，不建議日常使用。
...
這裡先取出 `a` 與 `b` 對應的值並完成加法，再將名稱 `total` 綁定到結果 `3`。

更精確的心智模型是：

> `=` 讓左側名稱綁定到右側運算結果。

不要把它理解成「把值複製進一個盒子」。學到 `list`、`dict` 與其他可變物件時，同一個物件可以同時被多個名稱參照：
...
1. **理解指定的心智模型**：`=` 是名稱綁定；`==` 才是相等比較。
2. **讓程式結構一眼可讀**：每層使用四個空白，並保持縮排一致。
3. **用註解說明原因**：`#` 是註解；三引號產生字串或 `docstring`。
4. **分清運算子的語意**：`/`、`//`、`%` 各自不同，尤其要測試負數。
5. **優先使用括號換行**：比反斜線續行容易閱讀、修改與格式化。
6. **先預測再執行**：比較預測與實際結果，有助於發現理解錯誤。
...
撰寫 Python 程式時，除了正確使用指定，還要分清運算語意、用縮排表達程式區塊，並寫下不會誤導讀者的註解。若預測與實際輸出不同，應保留程式碼、預測值與實際值，直接詢問教學助理釐清。
