...
從 Python 語法的角度看，`-3` 是負號運算子 `-` 作用於整數字面值 `3` 的結果。初學時不影響日常使用，但這能解釋某些運算優先順序：
...
這不是 Python 算錯，而是二進位浮點表示的限制。比較計算結果時，可以使用 `math.isclose()`：
...
`type()` 接收物件並回傳該物件的型別。這延續上一章的物件模型：型別屬於物件，不是名稱本身。
...
1. 先把常見內建型別分成數值、布林、序列、集合與映射。
2. 用「物件能否原地改變」區分 mutable 與 immutable。
3. 學會 `int` 的十進位、二進位、八進位與十六進位寫法。
4. 學會一般 `float` 與 `e` / `E` 科學記號。
5. 用 `j` 建立虛部，並以 `complex` 理解結果。
6. 每個範例都先預測值與型別，再用 `print()`、`type()` 驗證。
7. 最後補上實務警戒線：浮點誤差、精確的型別術語，以及 mutable 物件共用問題。
...
有任何一題無法解釋時，請把你的預測與實際輸出一起交給老師；差異本身就是下一個要修正的心智模型。
