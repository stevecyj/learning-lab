...
「實數集合加上純虛數集合」只會得到兩條軸，還會漏掉 `5 + 3i` 這類同時具有實部與虛部的點。複數包含的是整個平面。
...
這不是在替複數建立自然順序，而是在比較兩個 `float` 距離。
...
`cmath` 還提供複數的相位、極座標、指數、對數與三角函式。實際選用時，可以先看問題是否涉及複數：
...
7. **先問業務領域需不需要複數。** 複數常見於訊號處理、電機、控制、波動與科學計算；一般金額、計數或表格資料通常不該為了「型別更大」而轉成 `complex`。
...
這個「先預測、再執行、最後解釋」的循環，比只看懂範例更能確認自己真的掌握了觀念。
