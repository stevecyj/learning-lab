...
raw string 仍有自己的語法限制，不能簡化成「所有反斜線都會失效」。現階段若要在一般字串中顯示反斜線，使用 `\\` 最清楚。
...
但拆開後比較容易檢查原始輸入，也方便顯示精確的錯誤訊息。處理使用者輸入時，可以拆成四個步驟：
...
1. **把輸入視為不可信任的字串**：先清理、驗證與轉換型別，再進行計算。
...
7. 型別轉換只確認文字能否轉成數字；長方形的邊長還必須大於零。
...
`print()` 與 `input()` 是程式與使用者互動的基礎。程式不應假設輸入一定正確；將輸入、計算與輸出拆開，也會比較容易檢查。若實際結果與預測不同，可以把輸入內容、程式碼、錯誤訊息與預期結果一起交給教學助理分析。
