...
`--python 3.11` 會讓 uv 從第一步就建立一致的設定：
...
此時不必再執行 `uv python pin 3.11`，因為初始化時已寫入 `.python-version`。
...
Jupyter Notebook 的 `.ipynb` 檔案可以提交，自動產生的 `.ipynb_checkpoints/` 則應加入 `.gitignore`。提交前可先檢查：
...
### `.python-version` 內容為 `3.11`

代表：

> 當命令未以 `--python` 指定版本時，要求 uv 優先為這個專案選擇 Python 3.11。
...
`uv python find` 顯示 uv 依目前設定找到的 Python 直譯器；`sys.executable`
...
新專案先明確指定 Python。既有專案提交版本設定與鎖檔。換電腦後，安裝 uv，再執行同步即可。
...
`uv run` 會在執行命令前檢查 `uv.lock` 與 `.venv`，再在專案 `.venv` 中執行 JupyterLab。不必先手動啟用虛擬環境：

```sh
source .venv/bin/activate
```

如果只想臨時啟動 Jupyter，不把它加入專案依賴，可以使用 `uv run --with jupyter jupyter lab`。本專案已執行過
...
瀏覽器若沒有自動開啟，將終端機顯示的完整網址複製到瀏覽器。不要把 token 網址傳給其他人。

停止 JupyterLab 時，回到啟動它的終端機視窗按 `Ctrl+C`。只關閉瀏覽器分頁，Jupyter server 與 Notebook
kernel 可能仍在執行。

### 確認 Notebook 使用的 Python
...
1. 不會；uv 仍會依版本要求，從可用來源選擇 Python 直譯器。
